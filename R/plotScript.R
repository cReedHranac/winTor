...
Surv.plot <- function(x, c.string, res.agg = 25, dist.map = NULL,
                         save.name = NULL,  device.out = NULL,  ...){
  ## Create DataFrame (aggragation is mainly for the dev period)
  if(!is.null(res.agg)){ #aggratetion bits
    x.ag <- raster::aggregate(x, res.agg)
...
(surfaceNull.plot <- Surv.plot(surfaceSurv.null, survColors(5)))
(surfaceInf.plot <- Surv.plot(surfaceSurv.inf, survColors(5)))
