...
#### Cleaning data####
library(skimr)
s.dirty <- fread(file.path(serdp.newDat, "dat_Morpho.csv"))
## Manual adj for MYYU8
s.dirty[which(s.dirty$ID=="MYYU8"),"Body Mass"] <- 6.8

sWithFat <- s.dirty %>%
  filter(!is.na(`Fat Mass`)) %>%
  separate(Date,  c("d","m","y")) %>%
  filter(m %in% c("09","10","11")) %>%
  #as factors
  mutate(Sex = as.factor(Sex),
         `Reproductive Condition` = as.factor(`Reproductive Condition`),
         Age = as.factor(Age),
         `Site ID` = as.factor(`Site ID`),
         `State/Province` = as.factor(`State/Province`),
         Country = as.factor(Country), 
         Species = as.factor(Species)) %>%
  filter(Age == "Adult") # Remove YOY
...
library(lme4)
## Species effect
m1 <- lm(`Lean Mass` ~ Species, s.full)
summary(m1) ## yep

##
m2 <- lmer(`Lean Mass` ~ `Body Mass`*Sex + (1|Species), s.full)
summary(m2)


m3 <- lmer(`Lean Mass` ~ `Body Mass` + (1|Species/`Site ID`), s.full)
summary(m3)



m4 <- lm(`Lean Mass` ~ `Body Mass`, s.full)
summary(m4)
...
lm.out <- list()
for(i in 1:length(unique(s.full$Species))){
  a <- s.full %>%
    filter(Species == unique(s.full$Species)[[i]])
  
  lm.out[[i]] <- lm(`Lean Mass` ~ `Body Mass` , a)
  
  
}
lapply(lm.out, summary)
